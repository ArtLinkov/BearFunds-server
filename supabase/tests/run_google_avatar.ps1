# google_avatar suite runner (PowerShell) - Windows-native equivalent of run_rls.sh, so the
...
# google_avatar.test.sql into Postgres with ON_ERROR_STOP. Prefers a local psql; falls back to
...
Write-Host "==> Resetting to a virgin DB (the google_avatar suite assumes one)..."
...
if (Get-Command psql -ErrorAction SilentlyContinue) {
  Write-Host "==> Running google_avatar suite via local psql..."
  psql $DbUrl -v ON_ERROR_STOP=1 -q -f $Suite
} else {
  Write-Host "==> No local psql; running google_avatar suite via the db container..."
...
if ($LASTEXITCODE -ne 0) { Write-Error "google_avatar suite FAILED (psql exit $LASTEXITCODE)." }
Write-Host "==> google_avatar suite PASSED."
